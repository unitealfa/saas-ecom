...
- **`reglements_abonnement` :** UNIQUE(cle_operation), UNIQUE(contrepassation_de_id) hors NULL, UNIQUE(id,echeance_id). FK composite `(contrepassation_de_id,echeance_id)` → `reglements_abonnement(id,echeance_id)` ; appliquer aussi le même rattachement à `correction_de_id` lorsqu’il est renseigné. CHECK `contrepassation_de_id IS NULL OR contrepassation_de_id<>id`. Une contrepassation/correction ne peut donc jamais viser un règlement d’une autre échéance. Montant>0 pour un paiement, négatif uniquement pour son inverse exact. Une ligne validée est immuable ; correction par contrepassation puis nouvelle ligne, même échéance. annule_at réservé aux lignes non encore validées, exclues des sommes. Verrouiller l’échéance à la validation, interdire un solde négatif ou supérieur au montant dû sans traitement explicite du trop-perçu. Le moyen reste configurable ; aucune donnée de carte.
